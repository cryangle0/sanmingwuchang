...
describe('autumn storm look', () => {
  it('keeps the whole map readable, wet and raining', () => {
    ...
    // Darker storm pass (2026-09-13): the key, sky and exposure all came
    // down so the wood reads as weather rather than a lit stage, but the map
    // must stay readable — floors below keep it from going to mud.
    expect(AUTUMN_STORM.backgroundIntensity).toBeGreaterThanOrEqual(0.66);
    expect(AUTUMN_STORM.backgroundIntensity).toBeLessThanOrEqual(0.82);
    expect(AUTUMN_STORM.sunIntensity).toBeGreaterThanOrEqual(1.2);
    expect(AUTUMN_STORM.sunIntensity).toBeLessThanOrEqual(1.5);
    expect(AUTUMN_STORM.hemiIntensity).toBeGreaterThanOrEqual(1.1);
    expect(AUTUMN_STORM.hemiIntensity).toBeLessThanOrEqual(1.4);
    expect(AUTUMN_STORM.exposure).toBeGreaterThanOrEqual(1.0);
    expect(AUTUMN_STORM.exposure).toBeLessThanOrEqual(1.2);
    expect(AUTUMN_STORM.windLeafHigh).toBeGreaterThan(0.1);
